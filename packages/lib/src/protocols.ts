...
export const PROTOCOL_EXCLUSIONS: [number, number][] = [[GLYPH_FT, GLYPH_NFT]];
 ...
 */
export function validateProtocols(protocols: number[]): {
  valid: boolean;
  error?: string;
} {
  ...
  for (const [a, b] of PROTOCOL_EXCLUSIONS) {
    if (protocols.includes(a) && protocols.includes(b)) {
      return {
        valid: false,
        error: `${PROTOCOL_NAMES[a]} and ${PROTOCOL_NAMES[b]} are mutually exclusive`,
      };
    }
  }
    ...
    if (required) {
      for (const req of required) {
        if (!protocols.includes(req)) {
          return {
            valid: false,
            error: `${PROTOCOL_NAMES[protocol]} requires ${PROTOCOL_NAMES[req]}`,
          };
        }
      }
    }
  ...
  if (protocols.length === 1 && PROTOCOLS_REQUIRE_BASE.includes(protocols[0])) {
    return {
      valid: false,
      error: `${PROTOCOL_NAMES[protocols[0]]} cannot exist alone`,
    };
  }
