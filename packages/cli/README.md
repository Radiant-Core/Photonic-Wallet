...
Wallet files are JSON containing an AES-encrypted blob of the BIP-39 mnemonic
plus metadata used to derive keys. The minimum required fields are:
...
### Legacy wallets (pre-Photonic Wallet v3.0.0)

Wallets created before v3.0.0 derived keys at SLIP-0044 coin type **0**.
Modern wallets use coin type **512** (the registered Radiant ID).

If your wallet file was generated by an older version of Photonic Wallet
and the CLI reports an address mismatch — or transactions fail
"mandatory-script-verify (OP_EQUALVERIFY)" on broadcast — add an explicit
`coinType` field to the wallet JSON:
...
Omit `coinType` (or set it to `512`) for modern wallets.
